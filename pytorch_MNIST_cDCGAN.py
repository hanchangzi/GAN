...
fixed_z_ = fixed_z_.view(-1, 100, 1, 1).cuda()
fixed_y_label_ = torch.zeros(100, 10)
fixed_y_label_.scatter_(1, fixed_y_.type(torch.LongTensor), 1)
fixed_y_label_ = fixed_y_label_.view(-1, 10, 1, 1).cuda()
...
for epoch in range(train_epoch):
    ...
    y_real_ = torch.ones(batch_size).cuda()
    y_fake_ = torch.zeros(batch_size).cuda()
    for x_, y_ in train_loader:
        ...
        x_ = x_.cuda()
        y_fill_ = fill[y_].cuda()#给判别器的标签是３维张量，相比于全连接网络里面给的一个向量，这里可以看做把向量的每个点看做一张和原始图片一样大小的图片
        D_result = D(x_, y_fill_).squeeze()
        D_real_loss = BCE_loss(D_result, y_real_)

        z_ = torch.randn((mini_batch, 100)).view(-1, 100, 1, 1).cuda()#随机向量是100维
        ...
        y_ = (torch.rand(mini_batch, 1) * 10).type(torch.LongTensor).squeeze().cuda()
        y_label_ = onehot[y_].cuda()
        y_fill_ = fill[y_].cuda()
