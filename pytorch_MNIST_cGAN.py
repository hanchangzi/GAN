...
for epoch in range(train_epoch):
    ...
    for x_, y_ in train_loader:
        ...
        y_real_ = torch.ones(mini_batch).cuda()#判别器的真实标签
        y_fake_ = torch.zeros(mini_batch).cuda()#判别器的伪造标签
        y_label_ = torch.zeros(mini_batch, 10).cuda()
        y_label_.scatter_(1, y_.view(mini_batch, 1), 1)#生成01向量

        x_ = x_.view(-1, 28 * 28).cuda()
        D_result = D(x_, y_label_)#真实数据和标签
        D_real_loss = BCE_loss(D_result, y_real_)

        z_ = torch.rand((mini_batch, 100)).cuda()
        y_ = (torch.rand(mini_batch, 1) * 10).type(torch.LongTensor).cuda()
        y_label_ = torch.zeros(mini_batch, 10).cuda()
        y_label_.scatter_(1, y_.view(mini_batch, 1), 1)


        G_result = G(z_, y_label_)#随机向量和标签

        D_result = D(G_result, y_label_)#伪造数据和标签
        ...
        y_ = (torch.rand(mini_batch, 1) * 10).type(torch.LongTensor).cuda()
        y_label_ = torch.zeros(mini_batch, 10).cuda()
        y_label_.scatter_(1, y_.view(mini_batch, 1), 1).cuda()
